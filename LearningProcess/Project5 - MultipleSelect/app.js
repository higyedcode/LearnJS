...
const nr = episodes.length;
let lastChecked = -1;
for (let i = 1; i <= nr; i++) {
  episodeList.innerHTML += `
  <li>
            <label for="episode-${i}">
              <input type="checkbox" name="episode-${i}" id="episode-${i}" />
              <span>${i} || ${episodes[i - 1].name}</span>
            </label>
          </li>
  `;
}
const episodeLabels = episodeList.querySelectorAll("label");
console.log(episodeLabels);

episodeLabels.forEach((label, index) => {
    ...
    if (e.shiftKey) {
      ...
      if (lastChecked <= index) {
        for (let i = lastChecked + 1; i < index; i++) {
          episodeLabels[i].firstElementChild.checked = true;
        }
      } else {
        for (let i = lastChecked - 1; i > index; i--) {
          episodeLabels[i].firstElementChild.checked = true;
